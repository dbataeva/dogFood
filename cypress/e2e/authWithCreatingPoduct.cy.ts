import { PAGES_TEST_ID_MAP } from '../../src/app/components/pages/constants';
import { HEADER_TEST_ID_MAP } from '../../src/app/components/molecules/Header/constants';
import { SIGN_IN_PAGE_TEXT_MAP } from '../../src/app/components/pages/SignInPage/constants';
import { HEADER_ICON_BUTTONS_TEST_ID_MAP } from '../../src/app/components/atoms/HeaderIconButtons/constants';
import { CREATE_PRODUCT_FORM_TEST_ID_MAP } from '../../src/app/components/molecules/CreateProductForm/constants';
import { PRODUCT_CARD_ACTIONS_TEST_ID_MAP } from '../../src/app/components/atoms/ProductCardActions/constants';

describe('Проверка аторизации, создания продукта и проставления ему лайка', () => {
	beforeEach(() => cy.visit('/signIn'));

	const productName = `платье ${Date.now()}`;
	const productPrice = '1000';
	const productDiscount = '10';
	const productPicture =
		'https://spb.salon-love-forever.ru/upload/resize_cache/iblock/d65/120_180_2/Leda-midi.jpg';
	const productStock = '10';
	const productDescription = productName;

	it('должна открыться страница авторизации', () => {
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.signInPage}]`).should(
			'be.visible'
		);
	});

	it('не должна производится авторизация при невалидно заполненной форме входа', () => {
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.emailTextField}]`).type('aaa');
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.passwordTextField}]`).type('123');
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.submitButton}]`).click();

		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.emailTextField}]`).should(
			'contain',
			SIGN_IN_PAGE_TEXT_MAP.emailError
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.passwordTextField}]`).should(
			'contain',
			SIGN_IN_PAGE_TEXT_MAP.passwordError
		);
		cy.url().should('include', '/signIn');
	});

	it('не должна производится авторизация при неправильных введенных данных в форму входа', () => {
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.emailTextField}]`).type(
			'aaa@gmail.com'
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.passwordTextField}]`).type(
			'123123123'
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.submitButton}]`).click();

		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.emailTextField}]`).should(
			'not.contain',
			SIGN_IN_PAGE_TEXT_MAP.emailError
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.passwordTextField}]`).should(
			'not.contain',
			SIGN_IN_PAGE_TEXT_MAP.passwordError
		);
		cy.url().should('include', '/signIn');
	});

	it('должна производится авторизация при правильных введенных данных в форму входа', () => {
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.emailTextField}]`).type(
			'79228526659@yandex.ru'
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.passwordTextField}]`).type(
			'12345678'
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.submitButton}]`).click();

		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.emailTextField}]`).should(
			'not.contain',
			SIGN_IN_PAGE_TEXT_MAP.emailError
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.passwordTextField}]`).should(
			'not.contain',
			SIGN_IN_PAGE_TEXT_MAP.passwordError
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.homePage}]`).should('be.visible');
	});

	it('должна производится переадресация на страницу signIn после выхода из приложения', () => {
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.emailTextField}]`).type(
			'79228526659@yandex.ru'
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.passwordTextField}]`).type(
			'12345678'
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.submitButton}]`).click();
		cy.get(`[data-testid=${HEADER_ICON_BUTTONS_TEST_ID_MAP.quit}]`).click();

		cy.url().should('include', '/signIn');
	});

	it('должен создаваться продукт с возможностью поставить ему лайк', () => {
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.emailTextField}]`).type(
			'79228526659@yandex.ru'
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.passwordTextField}]`).type(
			'12345678'
		);
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.submitButton}]`).click();
		cy.get(`[data-testid=${HEADER_ICON_BUTTONS_TEST_ID_MAP.profile}]`).click();

		cy.get(`[data-testid=${CREATE_PRODUCT_FORM_TEST_ID_MAP.form}]`).should(
			'be.visible'
		);

		cy.get(`[data-testid=${CREATE_PRODUCT_FORM_TEST_ID_MAP.nameField}]`).type(
			productName
		);
		cy.get(`[data-testid=${CREATE_PRODUCT_FORM_TEST_ID_MAP.priceField}]`).type(
			productPrice
		);
		cy.get(
			`[data-testid=${CREATE_PRODUCT_FORM_TEST_ID_MAP.discountField}]`
		).type(productDiscount);
		cy.get(
			`[data-testid=${CREATE_PRODUCT_FORM_TEST_ID_MAP.picturesField}]`
		).type(productPicture);
		cy.get(`[data-testid=${CREATE_PRODUCT_FORM_TEST_ID_MAP.stockField}]`).type(
			productStock
		);
		cy.get(
			`[data-testid=${CREATE_PRODUCT_FORM_TEST_ID_MAP.descriptionField}]`
		).type(productDescription);
		cy.get(
			`[data-testid=${CREATE_PRODUCT_FORM_TEST_ID_MAP.submitButton}]`
		).click();

		cy.get(`[data-testid=${HEADER_TEST_ID_MAP.logo}]`).click();
		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.homePage}]`).should('be.visible');

		cy.get(`[data-testid=${HEADER_TEST_ID_MAP.searchBar}]`).type(productName);

		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.productMiniCard}]`)
			.should('contain', productName)
			.within(() =>
				cy
					.get(`[data-testid=${PRODUCT_CARD_ACTIONS_TEST_ID_MAP.nonLikedIcon}]`)
					.should('be.visible')
			);
		cy.get(
			`[data-testid=${PRODUCT_CARD_ACTIONS_TEST_ID_MAP.likeButton}]`
		).click();

		cy.get(`[data-testid=${PAGES_TEST_ID_MAP.productMiniCard}]`)
			.should('contain', productName)
			.within(() =>
				cy
					.get(`[data-testid=${PRODUCT_CARD_ACTIONS_TEST_ID_MAP.likedIcon}]`)
					.should('be.visible')
			);
	});
});
